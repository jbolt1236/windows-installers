...
function Context-ElasticsearchService($Expected) {
    ...
    Context "Elasticsearch service" {
        ...
        It "Service status is $($Expected.Status)" {
			...
			# BUG: Upgrading from Elasticsearch 5.5.0-2, to 5.x, the service is not started after upgrading.
			# Start the service if commanded to do so
			if ($Expected.StartIfNotRunning -and $Expected.Status -ne "Running" -and ($Expected.Status -eq "StopPending" -or $Expected.Status -eq "Stopped")) {
